    ...
    return (
      <>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--color-ink-primary)', marginBottom: 8 }}>Work order not found</div>
          ...
          <button onClick={onClose} style={{ padding: '10px 24px', borderRadius: 12, background: 'linear-gradient(135deg,var(--bos-color-brand-primary-deep),var(--bos-color-brand-primary))', color: 'white', border: 'none', fontSize: 13, fontWeight: 800, cursor: 'pointer' }}>Close</button>
        </div>
      </>
    );
  }
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* Drag handle */}
          <div style={{ position: 'absolute', top: 8, left: '50%', transform: 'translateX(-50%)', width: 36, height: 4, borderRadius: 2, background: 'var(--color-surface-border)' }} />

          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 4 }}>
            <div style={{ width: 4, height: 36, borderRadius: 2, background: stage.color }} />
            <div>
              <div style={{ fontSize: 17, fontWeight: 800, color: 'var(--color-ink-primary)', letterSpacing: '-0.02em', lineHeight: 1.2 }}>
                {toTitleCase(wo.name)}
              </div>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <button
              ...
              style={{ padding: '7px 14px', borderRadius: 10, background: 'rgba(15,118,110,0.08)', border: '1px solid rgba(15,118,110,0.2)', color: 'var(--bos-color-brand-primary-deep)', fontSize: 12, fontWeight: 800, cursor: 'pointer' }}>
              📊 Estimate
            </button>
            <button
            ...
            {dirty && !readOnly && (
              <button
                onClick={handleSave}
                disabled={saving}
                style={{ padding: '7px 16px', borderRadius: 10, background: saving ? 'var(--color-surface-border)' : 'linear-gradient(135deg,var(--bos-color-brand-primary-deep),var(--bos-color-brand-primary))', color: saving ? 'var(--bos-color-ink-tertiary)' : 'white', border: 'none', fontSize: 12, fontWeight: 800, cursor: saving ? 'default' : 'pointer', boxShadow: saving ? 'none' : '0 2px 8px rgba(15,118,110,0.3)' }}>
            ...
            <button onClick={requestClose} style={{ width: 32, height: 32, borderRadius: 8, border: '1px solid var(--color-surface-border)', background: 'white', color: 'var(--bos-color-ink-disabled)', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 }}>×</button>
          </div>
        </div>
        ...
        {linkingFolder && (
          <div style={{ padding: '10px 20px 12px', background: 'rgba(239,246,255,0.8)', borderBottom: '1px solid rgba(59,130,246,0.15)', display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
            ...
            <input
              ...
              style={{ flex: 1, padding: '8px 12px', borderRadius: 8, border: '1px solid rgba(3,105,161,0.3)', fontSize: 12, outline: 'none', background: 'white', color: 'var(--color-ink-primary)' }}
            />
            <button
            ...
            <button
              ...
              style={{ padding: '8px 10px', borderRadius: 8, border: '1px solid var(--color-surface-border)', background: 'white', color: 'var(--bos-color-ink-tertiary)', fontSize: 14, cursor: 'pointer', flexShrink: 0, lineHeight: 1 }}>
              ✕
            </button>
          </div>
        )}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 20px 40px' }}>
                      ...
                      {STAGES.map((s, idx) => {
                        ...
                        return (
                          <React.Fragment key={s.key}>
                            {idx > 0 && (
                              <div style={{ width: 20, height: 2, background: isPast ? STAGES[idx-1].color : 'var(--color-surface-border)', flexShrink: 0 }} />
                            )}
                            <div
                            ...
                            >
                              ...
                              <div style={{
                                ...
                                borderRadius: '50%',
                                background: isPast ? s.color : isCurrent ? s.color : 'var(--color-surface-border)',
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14, gridColumn: viewport === 'stacked' ? undefined : '1', order: viewport === 'stacked' ? 1 : undefined }}>
              ...
              {/* Job Details */}
              <div style={{ background: 'white', borderRadius: 14, border: '1px solid var(--color-surface-border)', padding: 18 }}>
                ...
                {!readOnly && (
                  <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10, flexWrap: 'wrap' }}>
                    {jobDetailsError && (
                      <span style={{ marginRight: 'auto', fontSize: 12, color: 'var(--color-red-700)', fontWeight: 700 }}>⚠️ {jobDetailsError}</span>
                    )}
                    ...
                    <button
                      ...
                      style={{ padding: '8px 14px', borderRadius: 9, border: '1px solid var(--color-surface-border)', background: 'white', color: 'var(--bos-color-ink-disabled)', fontSize: 12, fontWeight: 800, cursor: (!jobDetailsDirty || jobDetailsSaving) ? 'default' : 'pointer', opacity: (!jobDetailsDirty || jobDetailsSaving) ? 0.5 : 1 }}
                    >
                      Discard
                    </button>
                    <button
                      ...
                      disabled={!jobDetailsDirty || jobDetailsSaving}
                      style={{ padding: '8px 16px', borderRadius: 9, border: 'none', background: (!jobDetailsDirty || jobDetailsSaving) ? 'var(--color-surface-border)' : 'linear-gradient(135deg,var(--bos-color-brand-primary-deep),var(--bos-color-brand-primary))', color: (!jobDetailsDirty || jobDetailsSaving) ? 'var(--bos-color-ink-tertiary)' : 'white', fontSize: 12, fontWeight: 800, cursor: (!jobDetailsDirty || jobDetailsSaving) ? 'default' : 'pointer', boxShadow: (!jobDetailsDirty || jobDetailsSaving) ? 'none' : '0 2px 8px rgba(15,118,110,0.3)' }}
                    >
              ...
              <div style={{ background: 'white', borderRadius: 14, border: '1px solid var(--color-surface-border)', padding: 18 }}>
                ...
                {wo.requires_org_assignment && (
                  <div style={{ marginBottom: 14, padding: 14, borderRadius: 12, background: 'rgba(255,251,235,0.72)', border: '1px solid rgba(245,158,11,0.28)' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: viewport === 'stacked' ? '1fr' : '1fr 1fr', gap: 10, marginBottom: 12 }}>
                      {[
                        ...
                        <div key={label} style={{ padding: '8px 10px', borderRadius: 8, background: 'rgba(255,255,255,0.74)', border: '1px solid rgba(245,158,11,0.16)' }}>
                          ...
                          <div style={{ fontSize: 12, color: 'var(--color-ink-primary)', fontWeight: 700, marginTop: 3, overflowWrap: 'anywhere' }}>{value}</div>
                        </div>
                      ))}
                    </div>
                        ...
                        return (
                          ...
                          >
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                              <span style={{ fontSize: 12, fontWeight: 800, color: selected ? 'var(--bos-color-brand-primary-deep)' : 'var(--color-ink-primary)' }}>{org.name || org.company}</span>
                    ...
                    {orgRepairError && (
                      <div style={{ marginTop: 10, fontSize: 12, color: 'var(--color-red-700)', fontWeight: 700 }}>{orgRepairError}</div>
                    )}
                    ...
                    {!readOnly && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
                        <button
                          ...
                          style={{ padding: '8px 12px', borderRadius: 9, border: 'none', background: 'var(--bos-color-brand-primary-deep)', color: 'white', fontSize: 12, fontWeight: 800, cursor: selectedOrgId && !orgRepairSaving ? 'pointer' : 'default', opacity: selectedOrgId && !orgRepairSaving ? 1 : 0.5 }}
                        >
                        ...
                        <button
                          ...
                          style={{ padding: '8px 12px', borderRadius: 9, border: '1px solid rgba(15,118,110,0.3)', background: 'white', color: 'var(--bos-color-brand-primary-deep)', fontSize: 12, fontWeight: 800, cursor: !orgRepairSaving && !wo.org_id ? 'pointer' : 'default', opacity: !orgRepairSaving && !wo.org_id ? 1 : 0.5 }}
                        >
                      ...
                      {contactPeople.length > 1 && (
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8 }}>
                          {contactPeople.map(name => (
                            <span key={name} style={{ fontSize: 11, fontWeight: 700, color: 'var(--bos-color-brand-primary-deep)', background: 'rgba(15,118,110,0.08)', border: '1px solid rgba(15,118,110,0.18)', borderRadius: 999, padding: '4px 8px' }}>
                              {normalizeName(name)}
                            </span>
                          ))}
                        </div>
                ...
                {!readOnly && (
                  <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10, flexWrap: 'wrap' }}>
                    {customerSiteError && (
                      <span style={{ marginRight: 'auto', fontSize: 12, color: 'var(--color-red-700)', fontWeight: 700 }}>⚠️ {customerSiteError}</span>
                    )}
                    ...
                    <button
                      ...
                      style={{ padding: '8px 14px', borderRadius: 9, border: '1px solid var(--color-surface-border)', background: 'white', color: 'var(--bos-color-ink-disabled)', fontSize: 12, fontWeight: 800, cursor: (!customerSiteDirty || customerSiteSaving) ? 'default' : 'pointer', opacity: (!customerSiteDirty || customerSiteSaving) ? 0.5 : 1 }}
                    >
                      Discard
                    </button>
                    <button
                      ...
                      disabled={!customerSiteDirty || customerSiteSaving}
                      style={{ padding: '8px 16px', borderRadius: 9, border: 'none', background: (!customerSiteDirty || customerSiteSaving) ? 'var(--color-surface-border)' : 'linear-gradient(135deg,var(--bos-color-brand-primary-deep),var(--bos-color-brand-primary))', color: (!customerSiteDirty || customerSiteSaving) ? 'var(--bos-color-ink-tertiary)' : 'white', fontSize: 12, fontWeight: 800, cursor: (!customerSiteDirty || customerSiteSaving) ? 'default' : 'pointer', boxShadow: (!customerSiteDirty || customerSiteSaving) ? 'none' : '0 2px 8px rgba(15,118,110,0.3)' }}
                    >
                ...
                <div style={{ display: collapsed['crew'] ? 'none' : 'block' }}>
                  {islandCrew.length === 0 ? (
                    <div>
                      ...
                      <input style={{ fontSize: 13, padding: '7px 11px', borderRadius: 9, border: '1px solid var(--color-surface-border)', outline: 'none', width: '100%', boxSizing: 'border-box' as const }}
                      ...
                      {islandCrew.filter((c, i, arr) => arr.findIndex(x => x.name === c.name) === i).map(c => {
                        ...
                        return (
                          <button key={c.user_id} onClick={() => toggleCrew(c.name)} style={{
                            ...
                            border: sel ? '1px solid rgba(99,102,241,0.5)' : '1px solid var(--color-surface-border)',
                  ...
                  {!readOnly && (
                    <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10, flexWrap: 'wrap' }}>
                      {crewError && (
                        <span style={{ marginRight: 'auto', fontSize: 12, color: 'var(--color-red-700)', fontWeight: 700 }}>⚠️ {crewError}</span>
                      )}
                      {!crewDirty && !crewError && (
                      ...
                      <button
                        ...
                        style={{ padding: '8px 14px', borderRadius: 9, border: '1px solid var(--color-surface-border)', background: 'white', color: 'var(--bos-color-ink-disabled)', fontSize: 12, fontWeight: 800, cursor: (!crewDirty || crewSaving) ? 'default' : 'pointer', opacity: (!crewDirty || crewSaving) ? 0.5 : 1 }}
                      >
                        Discard
                      </button>
                      <button
                        ...
                        onClick={saveCrew}
                        disabled={!crewDirty || crewSaving}
                        style={{ padding: '8px 16px', borderRadius: 9, border: 'none', background: (!crewDirty || crewSaving) ? 'var(--color-surface-border)' : 'linear-gradient(135deg,var(--bos-color-brand-primary-deep),var(--bos-color-brand-primary))', color: (!crewDirty || crewSaving) ? 'var(--bos-color-ink-tertiary)' : 'white', fontSize: 12, fontWeight: 800, cursor: (!crewDirty || crewSaving) ? 'default' : 'pointer', boxShadow: (!crewDirty || crewSaving) ? 'none' : '0 2px 8px rgba(15,118,110,0.3)' }}
                      >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14, gridColumn: viewport === 'stacked' ? undefined : '1', order: viewport === 'stacked' ? 3 : undefined }}>
              ...
              {/* Procurement & Billing */}
              <div id="procurement-section" style={{ background: 'white', borderRadius: 14, border: '1px solid var(--color-surface-border)', padding: 18 }}>
                ...
                <div style={{ background: '#fafafa', borderRadius: 10, border: '1px solid var(--color-surface-border)', padding: '12px 14px', marginBottom: 10 }}>
                ...
                <div style={{ background: '#fafafa', borderRadius: 10, border: '1px solid var(--color-surface-border)', padding: '12px 14px', marginBottom: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                    ...
                    <button onClick={() => setShowAddQuote(p => !p)} style={{ fontSize: 11, padding: '4px 10px', borderRadius: 7, border: '1px solid var(--bos-color-brand-primary-deep)', background: showAddQuote ? 'var(--bos-color-brand-primary-deep)' : 'white', color: showAddQuote ? 'white' : 'var(--bos-color-brand-primary-deep)', cursor: 'pointer', fontWeight: 700 }}>
                  ...
                  {showAddQuote && (
                    <div style={{ background: 'white', borderRadius: 9, border: '1px solid var(--color-surface-border)', padding: '12px', marginBottom: 10 }}>
                      {/* Vendor search */}
                      <div style={{ marginBottom: 8, position: 'relative' }}>
                        ...
                        />
                        {showVendorDropdown && (
                          <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'white', border: '1px solid var(--color-surface-border)', borderRadius: 8, boxShadow: '0 4px 16px rgba(15,23,42,0.1)', zIndex: 500, maxHeight: 180, overflowY: 'auto' }}>
                            {vendorResults
                            ...
                            <div
                              ...
                              style={{ padding: '8px 12px', cursor: vendorCreating ? 'wait' : 'pointer', fontSize: 12, color: 'var(--bos-color-brand-primary-deep)', fontWeight: 700, borderTop: '1px solid #f1f5f9', opacity: vendorCreating ? 0.6 : 1 }}
                            >
                              {vendorCreating
                                ? 'Creating vendor…'
                                : vendorSearch.trim()
                        ...
                        {vendorCreateError && (
                          <div style={{ marginTop: 6, fontSize: 11, color: 'var(--color-red-700)', fontWeight: 600 }}>{vendorCreateError}</div>
                        )}
                      </div>
                      ...
                      <div style={{ marginBottom: 8 }}>
                        ...
                        {newQuote.line_items.map((li, i) => {
                          ...
                          <div key={i} style={{ display: 'grid', gridTemplateColumns: '3fr 1fr 1fr 1.2fr 80px 20px', gap: 6, marginBottom: 6, alignItems: 'center' }}>
                            ...
                            <span style={{fontSize:12,fontWeight:700,color:'var(--color-ink-primary)',textAlign:'right',paddingRight:4}}>${lineTotal.toFixed(2)}</span>
                        ...
                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:4, padding:'6px 0 2px' }}>
                          <button onClick={() => setNewQuote(p => ({...p, line_items: [...p.line_items, {description:'', quantity:'1', unit:'EA', unit_cost:''}]}))} style={{ fontSize: 11, color: 'var(--bos-color-brand-primary-deep)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 700, padding: 0 }}>+ Add Line Item</button>
                          <div style={{fontSize:13,fontWeight:800,color:'var(--color-ink-primary)'}}>
                      ...
                      <div style={{ marginBottom: 10 }}>
                        ...
                        {newQuote.quote_document_url ? (
                          <div style={{display:'flex',alignItems:'center',gap:6,fontSize:12,padding:'6px 0'}}>
                            <span>📎</span>
                            <a href={newQuote.quote_document_url} target="_blank" rel="noopener noreferrer" style={{color:'var(--bos-color-brand-primary-deep)',fontWeight:600}}>{newQuote.quote_document_name||'Document'}</a>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        }} style={{ flex:2, padding:'8px', borderRadius:8, border:'none', background:'linear-gradient(135deg,var(--bos-color-brand-primary-deep),var(--bos-color-brand-primary))', color:'white', fontSize:13, fontWeight:800, cursor:'pointer' }}>
                          Save Vendor Quote
                        </button>
                        <button onClick={() => { setShowAddQuote(false); setVendorSearch(''); }} style={{ flex:1, padding:'8px', borderRadius:8, border:'1px solid var(--color-surface-border)', background:'white', color:'var(--bos-color-ink-disabled)', fontSize:13, cursor:'pointer' }}>Cancel</button>
                      </div>
                    </div>
                  )}
                  ...
                  {procurementOrders.map(order => {
                    ...
                    return (
                      <div key={order.procurement_id} style={{background:'white',borderRadius:10,border:'1px solid var(--color-surface-border)',padding:'11px 13px',marginBottom:8}}>
                        {/* Header */}
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
                          <div>
                            <div style={{fontWeight:800,fontSize:13,color:'var(--color-ink-primary)'}}>{order.vendor_name||'Unknown Vendor'}</div>
                          ...
                          <div style={{display:'flex',gap:6,alignItems:'center'}}>
                            ...
                            {order.status !== 'CANCELLED' && (
                              ...
                              }} style={{background:'none',border:'1px solid var(--color-surface-border)',borderRadius:6,padding:'2px 7px',cursor:'pointer',fontSize:11,color:'var(--bos-color-ink-disabled)'}}>✕</button>
                            )}
                          </div>
                        </div>
                        ...
                        <div style={{background:'#fafafa',borderRadius:8,border:'1px solid #f1f5f9',overflow:'hidden',marginBottom:8}}>
                          {order.line_items.map((item: any, i: number) => (
                            <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'6px 10px',borderBottom:i<order.line_items.length-1?'1px solid #f1f5f9':'none',fontSize:12}}>
                              <span style={{color:'var(--color-ink-primary)'}}>{item.quantity}{item.unit && item.unit!=='EA' ? ' '+item.unit : ''} × {item.description}</span>
                              <span style={{fontWeight:700,color:'var(--color-ink-primary)'}}>${Number(item.line_total||0).toFixed(2)}</span>
                            </div>
                          ))}
                        ...
                        <div style={{marginBottom:8}}>
                          {order.quote_document_url ? (
                            <div style={{display:'flex',alignItems:'center',gap:6,fontSize:12}}>
                              <span>📎</span>
                              <a href={order.quote_document_url} target="_blank" rel="noopener noreferrer" style={{color:'var(--bos-color-brand-primary-deep)',fontWeight:600,textDecoration:'none'}}>
                            ...
                            <div style={{display:'flex',gap:8,alignItems:'center'}}>
                              <label style={{display:'flex',alignItems:'center',gap:4,fontSize:11,color:'var(--bos-color-brand-primary-deep)',fontWeight:700,cursor:'pointer',padding:'4px 10px',borderRadius:7,border:'1px dashed var(--bos-color-brand-primary-deep)',background:'white'}}>
                                📎 Attach PDF/Image
                                <input type="file" accept=".pdf,.jpg,.jpeg,.png,.heic" style={{display:'none'}} onChange={async e=>{
                                  const f=e.target.files?.[0]; if(!f) return;
                                  const fd=new FormData(); fd.append('file',f); fd.append('procurement_id',order.procurement_id); fd.append('wo_id',safeWo.id);
                              ...
                              <input placeholder="Paste Drive/URL link..." style={{flex:1,fontSize:11,padding:'4px 8px',borderRadius:7,border:'1px solid var(--color-surface-border)',outline:'none'}}
                                onBlur={async e=>{
                                  const url=e.target.value.trim(); if(!url) return;
                        ...
                        <div style={{display:'flex',gap:8}}>
                          {order.status==='VENDOR_QUOTED' && (
                            ...
                            }} style={{padding:'6px 12px',borderRadius:8,border:'1px solid var(--bos-color-brand-primary-deep)',background:'white',color:'var(--bos-color-brand-primary-deep)',fontSize:12,fontWeight:700,cursor:'pointer'}}>
                              ▶ Release Order
                            </button>
                          )}
                        ...
                        {isInspecting && (
                          <div style={{ marginTop:10, background:'#f0fdf4', borderRadius:8, border:'1px solid #bbf7d0', padding:'12px' }}>
                            ...
                            <textarea placeholder="Notes (optional)..." value={inspectionNotes} onChange={e => setInspectionNotes(e.target.value)}
                              style={{ width:'100%', borderRadius:7, border:'1px solid var(--color-surface-border)', padding:'7px 10px', fontSize:12, resize:'none', minHeight:50, boxSizing:'border-box', marginBottom:6 }} />
              ...
              {(() => {
                ...
                return (
                  <div style={{ background: 'white', borderRadius: 14, border: '1px solid var(--color-surface-border)', padding: 18 }}>
                    ...
                    <button onClick={addRow} style={{ width:'100%', padding:'10px', borderRadius:10, border:'1.5px dashed var(--bos-color-brand-primary-deep)', background:'transparent', color:'var(--bos-color-brand-primary-deep)', fontSize:13, fontWeight:700, cursor:'pointer' }}>
                      + Add Invoice
                    </button>
              ...
              <div style={{ background: 'white', borderRadius: 14, border: '1px solid var(--color-surface-border)', padding: 18 }}>
                ...
                <div style={{ display: collapsed['notes'] ? 'none' : 'block' }}>
                  ...
                  {!readOnly && (
                    <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10, flexWrap: 'wrap' }}>
                      {notesError && (
                        <span style={{ marginRight: 'auto', fontSize: 12, color: 'var(--color-red-700)', fontWeight: 700 }}>⚠️ {notesError}</span>
                      )}
                      ...
                      <button
                        ...
                        style={{ padding: '8px 14px', borderRadius: 9, border: '1px solid var(--color-surface-border)', background: 'white', color: 'var(--bos-color-ink-disabled)', fontSize: 12, fontWeight: 800, cursor: (!notesDirty || notesSaving) ? 'default' : 'pointer', opacity: (!notesDirty || notesSaving) ? 0.5 : 1 }}
                      >
                        Discard
                      </button>
                      <button
                        ...
                        onClick={saveNotes}
                        disabled={!notesDirty || notesSaving}
                        style={{ padding: '8px 16px', borderRadius: 9, border: 'none', background: (!notesDirty || notesSaving) ? 'var(--color-surface-border)' : 'linear-gradient(135deg,var(--bos-color-brand-primary-deep),var(--bos-color-brand-primary))', color: (!notesDirty || notesSaving) ? 'var(--bos-color-ink-tertiary)' : 'white', fontSize: 12, fontWeight: 800, cursor: (!notesDirty || notesSaving) ? 'default' : 'pointer', boxShadow: (!notesDirty || notesSaving) ? 'none' : '0 2px 8px rgba(15,118,110,0.3)' }}
                      >
              ...
              {wo.qbo_invoice_id && (
                <div style={{ background: 'white', borderRadius: 14, border: '1px solid var(--color-surface-border)', padding: 18 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--color-ink-primary)' }}>{(() => { const raw = wo.invoice_number || wo.qbo_invoice_id || ''; return /^\d{4}-\d{2}-\d{2}T/.test(raw) ? `(Draft - ${new Date(raw).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })})` : raw ? `#${raw}` : ''; })()}</div>
                    {(() => {
              ...
              <div style={{ background: 'white', borderRadius: 14, border: '1px solid var(--color-surface-border)', padding: 18 }}>
                ...
                <button onClick={() => toggleSection('job-files')} style={secBtn('job-files')}>
                  <span>Job Files{uploadedFiles.length > 0 && <span style={{ fontSize: 11, fontWeight: 800, color: 'var(--bos-color-brand-primary-deep)', background: 'rgba(15,118,110,0.08)', padding: '2px 8px', borderRadius: 999, border: '1px solid rgba(15,118,110,0.15)', marginLeft: 8 }}>{uploadedFiles.length}</span>}</span>
                  {chevron('job-files')}
                </button>
                ...
                <div
                  ...
                  style={{
                    border: `2px dashed ${isDraggingOver ? 'var(--bos-color-brand-primary)' : 'var(--color-surface-border)'}`,
                    borderRadius: 10,
                    padding: '14px 16px',
                ...
                >
                  ...
                  <div style={{ fontSize: 12, fontWeight: 700, color: isDraggingOver ? 'var(--bos-color-brand-primary-deep)' : 'var(--bos-color-ink-disabled)' }}>
        ...
        {dirty && !readOnly && (
          <div style={{
            ...
            background: 'white', borderTop: '1px solid var(--color-surface-border)',
            display: 'flex', justifyContent: 'flex-end', gap: 10,
          }}>
            <button onClick={() => { setDraft({}); setDirty(false); discardCustomerSite(); discardJobDetails(); discardNotes(); discardCrew(); onClose(); }}
              style={{ padding: '10px 20px', borderRadius: 10, border: '1px solid var(--color-surface-border)', background: 'white', color: 'var(--bos-color-ink-disabled)', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
              Discard
            </button>
            <button onClick={handleSave} disabled={saving}
              style={{ padding: '10px 24px', borderRadius: 10, background: saving ? 'var(--color-surface-border)' : 'linear-gradient(135deg,var(--bos-color-brand-primary-deep),var(--bos-color-brand-primary))', color: saving ? 'var(--bos-color-ink-tertiary)' : 'white', border: 'none', fontSize: 13, fontWeight: 800, cursor: saving ? 'default' : 'pointer', boxShadow: saving ? 'none' : '0 3px 10px rgba(15,118,110,0.3)' }}>
      ...
      {showCloseModal && (
        <>
          ...
          <div style={{
            ...
            <div style={{ fontSize:18, fontWeight:800, color:'var(--color-ink-primary)', marginBottom:6 }}>Close Work Order</div>
...
{(() => {
            ...
            <input type="number" step="0.5" min="0"
              ...
              style={{ width:'100%', padding:'10px 12px', borderRadius:10, border:'1px solid var(--color-surface-border)', fontSize:14, outline:'none', boxSizing:'border-box', marginBottom:16 }}
            />
            ...
            <textarea
              ...
              rows={3}
              style={{ width:'100%', padding:'10px 12px', borderRadius:10, border:'1px solid var(--color-surface-border)', fontSize:13, outline:'none', resize:'none', boxSizing:'border-box', marginBottom:20 }}
            />

            <div style={{ display:'flex', gap:10 }}>
              <button onClick={() => setShowCloseModal(false)}
                style={{ flex:1, padding:'12px', borderRadius:12, border:'1px solid var(--color-surface-border)', background:'white', color:'var(--bos-color-ink-disabled)', fontSize:13, fontWeight:700, cursor:'pointer' }}>
                Cancel
              </button>
              <button onClick={handleConfirmClose} disabled={closeSubmitting}
                style={{ flex:2, padding:'12px', borderRadius:12, border:'none',
                  background: closeSubmitting ? 'var(--color-surface-border)' : 'linear-gradient(135deg,#15803d,#16a34a)',
      ...
      {showDeclineModal && (
        <>
          ...
          <div style={{
            ...
            <div style={{ fontSize:18, fontWeight:800, color:'var(--color-ink-primary)', marginBottom:6 }}>Mark as Declined</div>
            ...
            <div style={{ display:'flex', gap:10 }}>
              ...
              <button onClick={handleConfirmDecline} disabled={declineSubmitting}
                style={{ flex:2, padding:'12px', borderRadius:12, border:'none',
                  background: declineSubmitting ? 'var(--color-surface-border)' : '#dc2626',
      ...
      {showScheduleModal && (
        <>
          ...
          <div style={{
            ...
            <div style={{ fontSize:18, fontWeight:800, color:'var(--color-ink-primary)', marginBottom:6 }}>Schedule Work Order</div>
            ...
            <input
              ...
              style={{ width:'100%', padding:'10px 12px', borderRadius:10, border:'1px solid var(--color-surface-border)', fontSize:14, outline:'none', boxSizing:'border-box', marginBottom:20 }}
            />
            ...
            <div style={{ display:'flex', gap:10 }}>
              ...
              <button
                ...
                style={{ flex:2, padding:'12px', borderRadius:12, border:'none',
                  background: scheduleSubmitting ? 'var(--color-surface-border)' : 'linear-gradient(135deg,#6d28d9,#7c3aed)',
