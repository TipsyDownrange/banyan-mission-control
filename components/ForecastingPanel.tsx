...
export default function ForecastingPanel() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #071722 0%, #0c2330 100%)', paddingBottom: 40 }}>
      <div style={{ background: 'linear-gradient(180deg, #0d1f2d 0%, rgba(13,31,45,0.95) 100%)', borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '20px 24px 16px', position: 'sticky', top: 0, zIndex: 10 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 5, maxWidth: 760, lineHeight: 1.5 }}>
      ...
      {loading && !data && <div style={{ textAlign: 'center', padding: 64, color: '#64748b' }}>Loading forecasting data…</div>}
        ...
        <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12 }}>
            {[
              ...
              <div key={String(label)} style={{ background: 'rgba(255,255,255,0.96)', border: '1px solid rgba(226,232,240,0.14)', borderRadius: 16, padding: 18 }}>
                <div style={{ fontSize: 10, fontWeight: 900, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#64748b' }}>{label}</div>
                <div style={{ marginTop: 8, fontSize: 28, fontWeight: 900, color: '#0f172a' }}>{value}</div>
                <div style={{ marginTop: 4, fontSize: 12, color: '#64748b' }}>{sub}</div>
              </div>
            ))}
          </div>
              ...
              {weeks.map((week, index) => (
                <div key={week.week_start} style={{ border: '1px solid rgba(148,163,184,0.18)', borderRadius: 14, padding: 14, background: index <= 2 ? 'rgba(14,165,233,0.08)' : 'rgba(20,184,166,0.08)' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginTop: 12 }}>
                    <div><div style={{ fontSize: 9, color: '#64748b', fontWeight: 800 }}>Needed</div><div style={{ color: '#f8fafc', fontWeight: 900 }}>{week.needed}</div></div>
                    <div><div style={{ fontSize: 9, color: '#64748b', fontWeight: 800 }}>Avail.</div><div style={{ color: '#f8fafc', fontWeight: 900 }}>{week.available}</div></div>
                    <div><div style={{ fontSize: 9, color: '#64748b', fontWeight: 800 }}>Buffer</div><div style={{ color: bufferColor(Number(week.buffer)), fontWeight: 900 }}>{week.buffer}</div></div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section style={{ background: 'rgba(15,23,42,0.54)', border: '1px solid rgba(148,163,184,0.16)', borderRadius: 18, overflow: 'hidden' }}>
            <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(148,163,184,0.12)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {lastRefresh && <div style={{ fontSize: 11, color: '#64748b' }}>Updated {lastRefresh.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}</div>}
            </div>
            <div style={{ display: 'grid', gap: 8, padding: 14 }}>
              {futureJobs.length === 0 && <div style={{ color: '#64748b', fontSize: 13, padding: 20, textAlign: 'center' }}>No unscheduled pipeline work returned.</div>}
              {futureJobs.slice(0, 18).map(job => (
                <div key={`${job.id}-${job.kID}`} style={{ display: 'grid', gridTemplateColumns: '100px 1fr 90px 120px', gap: 12, alignItems: 'center', padding: '12px 14px', borderRadius: 12, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(148,163,184,0.12)' }}>
                  <div style={{ fontFamily: 'monospace', fontSize: 12, color: '#94a3b8' }}>{job.kID}</div>
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>{[job.customer, job.island].filter(Boolean).join(' · ') || '—'}</div>
                  </div>
