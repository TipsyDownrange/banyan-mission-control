    ...
    <div style={{ padding: '32px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        {(['gc','gc_people','customers'] as const).map(k => (
          <button key={k} onClick={() => { setTab(k); setSearch(''); setSearchInput(''); setEditing(null); setExpanded(null); }} style={{ padding: '7px 16px', borderRadius: 999, fontSize: 11, fontWeight: 800, letterSpacing: '0.08em', textTransform: 'uppercase', border: tab === k ? '1px solid rgba(15,118,110,0.3)' : '1px solid #e2e8f0', background: tab === k ? 'rgba(240,253,250,0.96)' : 'white', color: tab === k ? '#0f766e' : 'var(--bos-color-ink-disabled)', cursor: 'pointer' }}>
              ...
              return (
                <div key={id}>
                  <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr 150px 150px 60px 80px', padding: '10px 20px', borderBottom: '1px solid #f8fafc', background: isExpanded ? 'rgba(240,253,250,0.3)' : 'white' }}>
                    ...
                    <div style={{ fontSize: 12, fontWeight: count > 5 ? 700 : 400, color: count > 10 ? '#0f766e' : 'var(--bos-color-ink-disabled)', display: 'flex', alignItems: 'center' }}>{count || '—'}</div>
