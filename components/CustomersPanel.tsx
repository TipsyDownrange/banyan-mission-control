...
const INP: React.CSSProperties = { width: '100%', padding: '7px 10px', borderRadius: 8, border: '1px solid rgba(15,118,110,0.3)', background: 'rgba(240,253,250,0.6)', fontSize: 12, color: 'var(--color-ink-primary)', outline: 'none' };
const FL = (label: string) => <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: 'var(--bos-color-ink-tertiary)', marginBottom: 3 }}>{label}</div>;
    ...
    <div style={{ padding: '32px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 24, padding: 18, borderRadius: 24, background: 'linear-gradient(135deg,rgba(255,255,255,0.98) 0%,rgba(240,249,255,0.92) 50%,rgba(248,250,252,0.96) 100%)', border: '1px solid rgba(148,163,184,0.18)', boxShadow: '0 4px 24px rgba(15,23,42,0.06)' }}>
        {[
          ...
          <div key={s.label} style={{ padding: '14px 16px', borderRadius: 18, background: 'rgba(255,255,255,0.78)', border: '1px solid rgba(226,232,240,0.95)' }}>
            ...
            <div style={{ marginTop: 6, fontSize: 28, fontWeight: 900, letterSpacing: '-0.05em', color: 'var(--color-ink-primary)', lineHeight: 1 }}>{s.value}</div>
      ...
      {showNew && (
        <div style={{ background: 'white', borderRadius: 20, border: '1px solid rgba(15,118,110,0.2)', boxShadow: '0 14px 30px rgba(15,23,42,0.06)', padding: '20px 24px', marginBottom: 16 }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--color-ink-primary)', marginBottom: 16 }}>New {tab === 'gc' ? 'GC Contact' : 'Customer'}</div>
      ...
      {!loading && (
        <div style={{ background: 'white', borderRadius: 20, border: '1px solid rgba(226,232,240,0.9)', boxShadow: '0 2px 12px rgba(15,23,42,0.04)', overflow: 'hidden' }}>
          ...
          <div style={{ maxHeight: 520, overflowY: 'auto' }}>
                ...
                <div key={id}>
                  <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr 150px 150px 60px 80px', padding: '10px 20px', borderBottom: '1px solid #f8fafc', background: isExpanded ? 'rgba(240,253,250,0.3)' : 'white' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                      {!isEditing ? (
                        <button onClick={() => startEdit(r)} style={{ padding: '4px 10px', borderRadius: 8, border: '1px solid rgba(15,118,110,0.2)', background: 'rgba(240,253,250,0.8)', color: 'var(--bos-color-brand-primary-deep)', fontSize: 10, fontWeight: 800, cursor: 'pointer', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Edit</button>
                      ) : (
            ...
            {records.length === 0 && <div style={{ padding: 32, textAlign: 'center', fontSize: 13, color: 'var(--bos-color-ink-tertiary)' }}>No records found</div>}
            {records.length > 100 && <div style={{ padding: '12px 20px', fontSize: 12, color: 'var(--bos-color-ink-tertiary)', textAlign: 'center' }}>Showing first 100 of {records.length} — use search to narrow</div>}
          </div>

          <div style={{ padding: '10px 20px', borderTop: '1px solid #f1f5f9', fontSize: 11, color: 'var(--bos-color-ink-tertiary)', background: 'rgba(248,250,252,0.5)' }}>
