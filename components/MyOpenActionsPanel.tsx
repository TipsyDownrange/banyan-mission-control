'use client';
/**
 * BAN-344a PM-V1.0-E (CORE) — My Open Actions cross-project view.
 *
 ...
 * action items across 8 projects" at a glance.
 */
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 14 }}>
        <div>
          ...
          <div style={{ fontSize: 22, fontWeight: 900, color: '#0f172a' }}>
            {userName ? `${userName.split(' ')[0]}'s ` : ''}{total} open {total === 1 ? 'action' : 'actions'}{' '}
            {projects > 0 && <span style={{ fontSize: 14, color: '#64748b', fontWeight: 700 }}>across {projects} {projects === 1 ? 'project' : 'projects'}</span>}
          </div>
        </div>
      </div>
      {sorted.length === 0 ? (
