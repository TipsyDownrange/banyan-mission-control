'use client';
/**
 * BAN-344a PM-V1.0-E (CORE) — Action Items surface.
 *
...
// Linked-source URL builder.  For MANUAL items (no source_entity_id) the
// chip is disabled and shows "—".  For other sources we link to the
// canonical tab inside ProjectsPanel rather than a deep entity URL, since
// 344a does not yet ship dedicated detail routes for every source trunk.
function linkedSourceFor(item: ActionItemRow, kid: string | null): string | null {
  if (!item.source_entity_id || item.source_entity_type === 'MANUAL') return null;
  if (!kid) return null;
  switch (item.source_entity_type) {
    case 'SUBMITTAL':       return `#/projects/${encodeURIComponent(kid)}/submittals`;
    case 'RFI':             return `#/projects/${encodeURIComponent(kid)}/rfis`;
    case 'VERBAL_AGREEMENT':return `#/projects/${encodeURIComponent(kid)}/verbal-agreements`;
    case 'MEETING':         return `#/projects/${encodeURIComponent(kid)}/meetings`;
    case 'PAY_APP':         return `#/projects/${encodeURIComponent(kid)}/pay-apps`;
    case 'TM_TICKET':       return `#/projects/${encodeURIComponent(kid)}/tm-tickets`;
    case 'CHANGE_ORDER':    return `#/projects/${encodeURIComponent(kid)}/cos`;
    case 'PUNCH_LIST_ITEM': return `#/projects/${encodeURIComponent(kid)}/punch-list`;
    default:                return null;
  }
}
...
export default function ActionItemsTab({ kID }: { kID: string }) {
  ...
  const [newActionRequired, setNewActionRequired] = useState('OTHER');
  ...
  const handleAdd = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewActionRequired('OTHER');
      setShowAdd(false);
      await fetchList();
    } finally {
      setSubmitting(false);
    }
  }, [newTitle, newDescription, newPriority, newActionRequired, newDueDate, kID, fetchList]);
    ...
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10, marginBottom: 14 }}>
        {[
          ...
          ['Completed', data.summary.by_status.COMPLETED ?? 0],
        ].map(([label, value]) => (
      ...
      {showAdd && (
        <form onSubmit={handleAdd} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12, padding: 14, marginBottom: 12, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="What needs to happen?" style={{ ...inputStyle, gridColumn: '1 / span 2' }} required maxLength={300} />
          <input value={newDescription} onChange={(e) => setNewDescription(e.target.value)} placeholder="Details (optional)" style={{ ...inputStyle, gridColumn: '1 / span 2' }} />
          <select value={newPriority} onChange={(e) => setNewPriority(e.target.value)} style={selectStyle}>
            {['URGENT', 'HIGH', 'MEDIUM', 'LOW'].map((p) => <option key={p} value={p}>Priority: {p}</option>)}
          </select>
          <select value={newActionRequired} onChange={(e) => setNewActionRequired(e.target.value)} style={selectStyle}>
            {['REVIEW', 'RESPOND', 'APPROVE', 'SUBMIT', 'FOLLOW_UP', 'CLOSE_OUT', 'OTHER'].map((a) => <option key={a} value={a}>Action: {a}</option>)}
          </select>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {filtered.length === 0 ? (
          ...
          const sourceLink = linkedSourceFor(it, data.engagement?.kid ?? null);
          return (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'minmax(220px, 2fr) 120px 110px 130px 200px', gap: 10, alignItems: 'center' }}>
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {it.action_required ?? '—'} ·{' '}
                    {sourceLink ? (
                      <a href={sourceLink} style={{ color: '#0f766e', fontWeight: 700, textDecoration: 'none' }}>{SOURCE_LABELS[it.source_entity_type] ?? it.source_entity_type} →</a>
                    ) : (
                      SOURCE_LABELS[it.source_entity_type] ?? it.source_entity_type
                    )}
                  </div>
                </div>
