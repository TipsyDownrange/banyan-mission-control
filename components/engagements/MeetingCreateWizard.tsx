            ...
            <div key={a.key} style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: 10, padding: 10, marginTop: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button type="button" onClick={() => setAttendees((p) => p.filter((x) => x.key !== a.key))} style={{ ...smallButtonStyle, background: '#fef2f2', color: '#b91c1c', borderColor: '#fecaca' }}>Remove</button>
                </div>
              </div>
              {a.is_kula_user ? (
                <select
