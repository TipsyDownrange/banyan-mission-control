...
// GET — get product line summary cards with article counts
export async function GET() {
  try {
    ...
    // Count articles per product line
    const countMap: Record<string, number> = {};
    for (const a of articles) {
      if (a.product_line) {
        countMap[a.product_line] = (countMap[a.product_line] || 0) + 1;
      }
    }

    const withCounts: KBProductLine[] = productLines.map(pl => ({
      ...pl,
      article_count: countMap[pl.name] || 0,
    }));
