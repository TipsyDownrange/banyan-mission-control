...
// Body: { template_name: string, steps: { step_name, default_hours, category, notes }[] }
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    // Identify row indices (1-based, sheet rows start at row 2 so add 2)
    const matchingIndices: number[] = [];
    for (let i = 0; i < allRows.length; i++) {
      if (allRows[i][0] === template_name) matchingIndices.push(i);
    }

    // We'll rebuild: remove old rows, insert new ones at same position
    // Strategy: keep all non-matching rows, then append new steps for this template at the end
    // Simpler approach: clear the full sheet content and rewrite it
...
// PATCH — rename a template, or update a single step (full step list replacement for now)
// Body: { old_name: string, new_name: string } OR { template_name: string, steps: [...] }
