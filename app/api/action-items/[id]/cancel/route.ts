/**
 * BAN-344 PM-V1.0-E — POST /api/action-items/[id]/cancel
 *
 * Transitions OPEN / IN_PROGRESS / DEFERRED → CANCELLED.  Caller supplies
 * a reason (recorded as auto_closed_reason for audit symmetry with
 * subscriber-driven AUTO_CLOSED rows).
 */
...
export async function POST(
  ...
  const result = await executeActionItemTransition({
    ...
    actorUserId,
    reason,
    extraUpdates: { auto_closed_reason: reason },
  });
