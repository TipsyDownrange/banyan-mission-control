...
describe('project detail sidebar navigation regression', () => {
  ...
  it('opens project detail on Overview so the financial summary is visible by default', () => {
    ...
    expect(source).toContain("useState<'overview'|'submittals'|'rfis'|'verbal-agreements'|'cos'|'pay-apps'|'tm-tickets'|'punch-list'|'budget'|'work-breakdown'|'matrix'|'activity'>('overview')");
