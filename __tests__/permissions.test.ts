...
describe('ROLE_PERMISSIONS_DEFAULTS', () => {
  ...
  it('grants no War Room permissions to every other documented role', () => {
    const exclude = new Set(['business_admin', 'super_admin']);
...
describe('ALL_ROLE_PERMISSIONS', () => {
  it('lists every RolePermission union member exactly once', () => {
    ...
    const expected: RolePermission[] = ['WARROOM_VIEW', 'WARROOM_TASK_WRITE'];
...
describe('hasPermission', () => {
  ...
  it('returns false for pm / field / estimator / sales / catalog_admin (defaults)', () => {
