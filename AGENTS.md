...
## PR Review Checklist (ADR-043)

Every PR generated by an AI lane (Codex, Claude Code, Kai, XO) must pass the 7-point review checklist in [ADR-043](https://drive.google.com/file/d/1Ha0tvyHTaIpjBtBn4rTLveu0m3sLf5Oh/view) before merge. The PR template at `.github/PULL_REQUEST_TEMPLATE.md` auto-populates the checklist in every new PR.

Failed item = REQUEST CHANGES, not merge with reservations. Self-scoring of each checklist point is required in the DONE REPORT.
