-- BAN-344a PM-V1.0-E (CORE) — Action Item Tracker entity + manual creation
-- Source: PM Trunk v1.0 §9 Action Item Tracker.
--
-- CORE-ONLY scope (per packet split):
--   - action_items entity
--   - manual creation paths
--   - ACTION_ITEM_CREATED + ACTION_ITEM_STATE_CHANGED Pattern A events
--
-- EXPLICITLY OUT OF SCOPE for 344a (lands in 344b):
--   - Source-trunk subscriber wiring
--   - Auto-close logic
--   - ACTION_ITEM_CLOSED_AUTO event
--   - AUTO_CLOSED status value
--
-- source_event_type and source_entity_id are nullable on this migration so
-- 344b can populate them from the subscriber pass without a destructive
-- ALTER. Additive per ADR-026.
...
-- NOTE: AUTO_CLOSED is intentionally NOT in the 344a status set.  344b will
-- add it via ALTER TYPE ... ADD VALUE.
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS public.action_items (
  ...
  -- Nullable in 344a (no source events feed action_items yet).  344b
  -- backfills these from the subscriber pass.
...
-- A MANUAL row may carry source_entity_id = NULL; non-MANUAL rows must
-- supply one (344b enforces this from the subscriber side).  The CHECK is
-- additive and allows the 344a manual flow to insert NULL freely.
ALTER TABLE public.action_items
  DROP CONSTRAINT IF EXISTS action_items_manual_source_id_optional;
ALTER TABLE public.action_items
  ADD CONSTRAINT action_items_manual_source_id_optional
  CHECK (
    source_entity_type = 'MANUAL' OR source_entity_id IS NOT NULL
  ) NOT VALID;
ALTER TABLE public.action_items VALIDATE CONSTRAINT action_items_manual_source_id_optional;
--> statement-breakpoint
...
-- ── Extend BAN-293 Activity Spine event_type CHECK with the two 344a
--    Pattern A events.  ACTION_ITEM_CLOSED_AUTO is reserved for 344b and
...
ALTER TABLE public.field_events
  ...
  CHECK (
    event_type IS NULL OR event_type IN (
      ...
      'ACTION_ITEM_CREATED','ACTION_ITEM_STATE_CHANGED',
