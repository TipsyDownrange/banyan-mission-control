/**
 * BAN-344 PM-V1.0-E — Action Item Tracker route helpers (parsers + shared
 * select fragments).  Mirrors lib/pm/meetings/route-utils.ts.
 */
...
/**
 * Compute the natural action_required token for a row, used as a hint when
 * source events do not specify one explicitly.  Pure function — exported
 * here so unit tests can pin the policy.
 */
export function defaultActionRequiredFor(
  ...
  switch (entityType) {
    ...
    case 'VERBAL_AGREEMENT': return 'CONFIRM';
    ...
    case 'WARRANTY_CLAIM': return 'TRIAGE';
    case 'MANUAL': return 'FOLLOW_UP';
  }
}
