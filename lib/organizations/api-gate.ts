 ...
 *
 * ORG-PERMISSIONS dispatch (2026-05-19, peer migration following the
 * WARROOM-PERMISSIONS / KB-PERMISSIONS / CONTACTS-PERMISSIONS template):
 * migrated from the hardcoded ORGANIZATIONS_WRITE_ROLES set (PR #189) to
 * the env-overridable RolePermission system in lib/permissions.ts.
 * Widening organization access no longer requires a code change + PR +
 * deploy — set ROLE_PERMISSIONS_JSON in Vercel instead.
 *
 ...
 * (ORG_VIEW / ORG_WRITE).  Retained as a backward-compat export so anything
 ...
 * Delegates to passPermissionGate(ORG_VIEW).  Default grants every documented
 * role except 'none', preserving the prior "any signed-in kulaglass.com user
 * with a resolved role" behavior; ContactAutocomplete / OrganizationAutocomplete
